...
// time: O(ElogV).
ll Kruskal(vector<iii>& edges, ll n, vector<iii>& res) {
...
int main() {
    ...
    for (int i = 0; i < a; ++i) {
        for (int j = 0; j < b; ++j) {
            ...
            mat2[j] = h;
            if(i>0){
                ii edge1(i*b+j,mat2[j]-mat1[j]);
                edges[(i-1)*b+j].push_back(edge1);
                ii edge2((i-1)*b+j,mat1[j]-mat2[j]);
                edges[i*b+j].push_back(edge2);
            }
            if(j>0){
                ii edge1(i*b+j,mat2[j]-mat2[j-1]);
                edges[i*b+j-1].push_back(edge1);
                ii edge2(i*b+j-1,mat2[j-1]-mat2[j]);
                edges[i*b+j].push_back(edge2);
            }
