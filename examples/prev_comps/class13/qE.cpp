...
bool onPoint(vector<point> cops,int x,int y){
    for (int i = 0; i < cops.size(); ++i) {
        if(cops[i].x == x && cops[i].y == y)
            return true;
    }
    return false;
}

bool onLine(vector<point> cops,int x,int y){
    for (int i = 0; i < cops.size()-1; ++i) {
        point c;
        if(distToLineSegment(point(x,y),cops[i],cops[i+1], c) < 0.0001){
            return true;
        }
    }
    return false;
...
int main() {
    ...
    while(cin >> c >> r >> o) {
        ...
        if(c >= 3) {
            hullCops = convex_hull(cops);
            hullCops.push_back(hullCops.at(0));
        }
        vector<point> robs;
        vector<point> hullRobs;
        ...
        if(r>=3) {
            hullRobs = convex_hull(robs);
            hullRobs.push_back(hullRobs.at(0));
        }

        for (int i = 0; i < o; i++) {
            int x, y;
            cin >> x >> y;
            if (c>=3 && (pointInPoly(hullCops, x, y) || onPoint(hullCops,x,y) || onLine(hullCops,x,y))) {
                printf("     Citizen at (%d,%d) is safe.\n",x,y);
            } else if (r>=3 && (pointInPoly(hullRobs, x, y) || onPoint(hullRobs,x,y)|| onLine(hullRobs,x,y))) {
