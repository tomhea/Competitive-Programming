...
int main() {
    ...
    vector<point> hull = convex_hull(red);
    ...
    for(int i = 0; i<B; i++){
        ...
        cin >> x >> y;
        point p(x,y);
//        bool flag = pointInPoly(hull,p.x,p.y);
//        printf("res:%d\n",flag);
        if(pointInPoly(hull,p.x,p.y))
            count++;
//        if(flag != pointInPoly2(hull,p.x,p.y)) {
//            printf("STOOOOOP:%f %f, our: %d should:%d\n", p.x, p.y, flag, pointInPoly2(hull, p.x, p.y));
//            pointInPoly(hull, p.x, p.y);
//        }
    }
    printf("%d\n",count);
