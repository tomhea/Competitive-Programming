...
bool compare(pair<char*, int> i1, pair<char*, int> i2)
{
    return strcmp(i1.first, i2.first) < 0;
}
...
//        T[l++] = '$';
        ...
        for (int j = 0; j < S.n; ++j) {
//            if(strcmp(T+S.SA[j],"$") == 0) continue;
            ss.emplace_back(T+S.SA[j],S.SA[j]);
        }
        stable_sort(ss.begin(), ss.end(),compare);
        bool first = true;
        for (int i = 0; i < n; ++i) {
            if (!first) printf(" ");
            printf("%d",ss[qs[i]].second);
            first = false;
        }
        printf("\n");
    }
